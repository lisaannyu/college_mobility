...
table_2 %>%
  ...
  summarize(percent_par_q1 = mean(par_q1),
            percent_kq5_pq1 = mean(mr_kq5_pq1)) %>% 
  mutate(percent_kNOTq5_pq1 = percent_par_q1 - percent_kq5_pq1) %>% 
  ...
  gather(key = "metric", value = "percent", starts_with("percent")) %>% 
  ggplot(aes(tier_name, percent, fill = metric)) +
  geom_col() +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(x = "Tier",
       y = "Percent with Parental Income in bottom 20%") +
...
table_2 %>%
  group_by(tier_name) %>% 
  summarize(percent_par_q1 = mean(par_q1),
            percent_ktop1_pq1 = mean(mr_ktop1_pq1)) %>% 
  mutate(percent_kNOTtop1_pq1 = percent_par_q1 - percent_ktop1_pq1) %>% 
  select(-percent_par_q1) %>% 
  mutate(tier_name = factor(tier_name,
                            levels = c("Ivy Plus", 
                                       "Other elite schools (public and private)",
  ...
  scale_y_continuous(labels = scales::percent)
```
Next Steps: create a visualization that gives an idea of the scale of these MR numbers: ktop1, pq1