...
## differences in p: which vars are in table_2 but not table_10?
```{r}
...
```{r}
mean(is.na(table_preds$mr_kq5_pq1))
...
table_2 %>%
  ...
  # filter(tier_name == "Other elite schools (public and private)")
  count(tier_name)

table_2 %>% 
  filter(!is.na(mr_ktop1_pq1))
```
...
## Are these variables missing completely at random? 
```{r}
table_preds_NA <- table_preds
table_preds_NA[table_preds_NA == -1] <- NA
```

### t-test
```{r}
t.test(table_preds_NA %>% filter(ipeds_enrollment_2000_NA == 1) %>% .$asian_or_pacific_share_fall_2000,
       table_preds_NA %>% filter(ipeds_enrollment_2000_NA == 0) %>% .$asian_or_pacific_share_fall_2000)

sum(is.na(table_preds_NA %>% filter(ipeds_enrollment_2000_NA == 1) %>% .$asian_or_pacific_share_fall_2000))
```

```{r}
t.test(table_preds_NA %>% filter(sat_avg_2013_NA == 1) %>% .$exp_instr_pc_2000,
       table_preds_NA %>% filter(sat_avg_2013_NA == 0) %>% .$exp_instr_pc_2000)

sum(!is.na(table_preds_NA %>% filter(sat_avg_2013_NA == 1) %>% .$exp_instr_pc_2000))
```

```{r}
table_preds_NA %>% select(one_of(vars_w_missingness)) %>% 
  purrr::map_int(~ sum(is.na(.)))
```

### chi-square test
```{r}
chisq.test(table_preds_NA$sat_avg_2013_NA,
           table_preds_NA$tier_name)

table_preds_NA %>% 
  group_by(tier_name) %>% 
  summarize(sat_avg_2013_NA = mean(sat_avg_2013_NA),
            n = n()) %>% 
  arrange(desc(sat_avg_2013_NA))
```

```{r}
missing_chisq <- tibble()
for (var in vars_w_missingness) {
  missing_chisq <- bind_rows(
    missing_chisq,
    tibble(
      variable = var,
      p = chisq.test(table_preds_NA[var] %>% .[[1]],
                     table_preds_NA$tier_name)$`p.value`
      ))
}
...
# SAT, Sticker Price, Avg Fac Salary
```
...
# Top 10 coefficients
# taxrate, lf_d_2000_1980, asian_or_pacific_share_fall_2000, poor_share,
...
table_2 %>%
  ...
  mutate(tier_name = factor(tier_name,
                            levels = c("Ivy Plus", 
